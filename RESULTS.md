...
phases', which flattens everything else to invisible on a linear scale) and `latency-percentiles.png`
(phase2's p50/p95/p99 vs concurrency, chosen as the most complete real-pipeline measurement). A
`throughput-vs-replicas.png` chart exists in code (`report.ThroughputVsReplicas`) but is not
generated in this run — Phase 4's scaling sweep is BLOCKED (see above), and the chart function
correctly refuses to render from BLOCKED/absent data rather than fabricate a curve.
